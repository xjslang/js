...
func (node *SwitchStmt) SelfClosing() bool {
	return true
}
...
func PrintSwitchStmt(p *printer.Printer, node *SwitchStmt) {
	p.LnPrint(node.Layout.Switch)
	p.SpPrint(node.Layout.Lparen)
	p.Print(node.Expr, node.Layout.Rparen)
	p.SpPrint(node.Layout.Lbrace)
	p.IncreaseIndent()
	for _, stmt := range node.Clauses {
		switch v := stmt.(type) {
		case *SwitchCaseStmt:
			printSwitchCaseStmt(p, v)
		case *SwitchDefaultStmt:
			printSwitchDefaultStmt(p, v)
		}
	}
	p.DecreaseIndent()
	p.LnPrint(node.Layout.Rbrace)
}

func printSwitchCaseStmt(p *printer.Printer, node *SwitchCaseStmt) {
	p.LnPrint(node.Layout.Case)
	p.SpPrint(node.Expr)
	p.Print(node.Layout.Colon)
	p.IncreaseIndent()
...
func printSwitchDefaultStmt(p *printer.Printer, node *SwitchDefaultStmt) {
	p.LnPrint(node.Layout.Default)
	p.Print(node.Layout.Colon)
	p.IncreaseIndent()
	for _, stmt := range node.Stmts {
		p.Print(stmt)
